...
def initialise_review_dataset(directory, schedule_length):
    ...
    if os.path.exists(fn):
        ...
        unreviewed = dset.query("reviewed == False")
        if len(unreviewed) > 0:
            schedule_index = unreviewed["schedule"].min()
        else:
            schedule_index = 0
    else:

        filenames = glob.glob(directory + "/*.*")
        # TODO: keep only filenames with image extensions
        ...
        num_schedule = int(np.ceil(len(dset) / schedule_length))
        dset["schedule"] = np.array([[i for _ in range(schedule_length)] for i in range(num_schedule)]).flatten()[:len(dset)]
        schedule_index = 0

    return (dset, schedule_index)
...
class App(QMainWindow):
    ...
    def __init__(self, parser_args):
        ...
        self.schedule_size = self.num_rows * self.num_cols
        ...
        self.reviews, self.schedule_index = initialise_review_dataset(self.directory, self.schedule_size)
        self.max_schedule_index = self.reviews["schedule"].max()
    ...
    def update_montage_panel(self):
        ...
        # TODO: fix slice error 
        self.currently_showing_images = self.reviews.loc[self.reviews["schedule"] == self.schedule_index]
        ...
        for r in range(self.num_rows):
            for c in range(self.num_cols):
                ...
                if n < len(self.currently_showing_images):
                    ...
                    # Flagged images now load as toggled/red
                    if self.currently_showing_images.loc[index, "score"] == -1:
                        l.toggle()
    ...
    def next(self):
        ...
        if self.schedule_index > self.max_schedule_index:
    ...
    def previous(self):
        ...
        if self.schedule_index < 0:
            self.schedule_index = self.max_schedule_index
